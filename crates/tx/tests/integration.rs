...
fn sign_transaction_input_with_dilithium2() {
    ...
    let (pk_bytes, sk_bytes) = Dilithium2Scheme::keygen_from_seed(&[42; 32])
...
fn complete_transaction_workflow() {
    ...
    let (sender_pk, sender_sk) = Dilithium2Scheme::keygen_from_seed(&[1; 32])
        .expect("sender keygen");
    let (receiver_pk, _) = Dilithium2Scheme::keygen_from_seed(&[2; 32])
        .expect("receiver keygen");
...
fn transaction_with_mixed_algorithms() {
    ...
    let (ed_pk, ed_sk) = Ed25519Stub::keygen_from_seed(&[33; 32])
    ...
    let (dil_pk, dil_sk) = Dilithium2Scheme::keygen_from_seed(&[44; 32])
        .expect("dilithium2 keygen");
...
fn transaction_serialization() {
    ...
    // Deserialize
    let deserialized: Tx = codec::from_slice_cbor(&serialized)
