...
use crypto::{AlgTag, Dilithium2Scheme, PublicKey, SecretKey, Signature, SignatureScheme, context, sign, verify};

/// Fuzz test: verify should handle arbitrary signature bytes gracefully
#[test]
fn fuzz_verify_random_signatures() {
    let (pk, _sk) = Dilithium2Scheme::keygen_from_seed(&[42; 32])
    ...
    for (i, bytes) in test_cases.iter().enumerate() {
        ...
        assert!(result.is_err(), 
            "Test case {} with {} bytes should be rejected", i, bytes.len());
    }
}
...
fn fuzz_verify_random_messages() {
    ...
    for (i, msg) in test_messages.iter().enumerate() {
        ...
        assert!(result.is_err(),
            "Test case {} with {} byte message should be rejected", i, msg.len());
    }
}
...
fn fuzz_sign_message_sizes() {
    ...
    let test_sizes = vec![0, 1, 31, 32, 33, 64, 127, 128, 255, 256, 1023, 1024, 4095, 4096, 8191, 8192];
    ...
    for size in test_sizes {
        ...
        if size > 0 {
            ...
            modified[0] ^= 0x01;
            assert!(verify(&modified, &pk_obj, &sig, context::TX).is_err(),
                "modified {} byte message should be rejected", size);
        }
    }
}
...
fn fuzz_verify_corrupted_keys() {
    ...
    for flip_pos in [0, pk.len() / 4, pk.len() / 2, pk.len() * 3 / 4, pk.len() - 1] {
        ...
        assert!(result.is_err(),
            "verification with corrupted key at position {} should fail", flip_pos);
    }
}
...
fn fuzz_trait_api_edge_cases() {
    let (pk, sk) = Dilithium2Scheme::keygen_from_seed(&[0; 32])
        .expect("all-zero seed should work");
    
    // All-zero message
    let msg = vec![0u8; 1024];
    let sig = Dilithium2Scheme::sign(&sk, &msg)
        .expect("signing all-zero message should succeed");
    assert!(Dilithium2Scheme::verify(&pk, &msg, &sig),
        "verifying all-zero message should succeed");
    
    // All-ones message
    let msg = vec![0xFFu8; 1024];
    let sig = Dilithium2Scheme::sign(&sk, &msg)
        .expect("signing all-ones message should succeed");
    assert!(Dilithium2Scheme::verify(&pk, &msg, &sig),
        "verifying all-ones message should succeed");
    
    // Alternating pattern
    let msg: Vec<u8> = (0..1024).map(|i| if i % 2 == 0 { 0xAA } else { 0x55 }).collect();
    let sig = Dilithium2Scheme::sign(&sk, &msg)
        .expect("signing alternating pattern should succeed");
    assert!(Dilithium2Scheme::verify(&pk, &msg, &sig),
