...
fn create_test_db(tip_height: u64) -> (TempDir, Store) {
    ...
    let store = Store::open_with_tuning(db_path, DbTuning::development())
...
fn test_snapshot_roundtrip_small() {
    let (db_dir, store) = create_test_db(100);
    let snapshot_dir = TempDir::new().expect("failed to create snapshot dir");
    
    // Create snapshot
    let manager = SnapshotManager::new(snapshot_dir.path())
    ...
    assert!(snapshot_path.extension().unwrap() == "gz", "snapshot should be gzipped");
    ...
    assert_eq!(hex::encode(metadata.parse_tip_hash().unwrap()), hex::encode(tip_before.hash));
    ...
    let tip_after = restored_store.tip().expect("failed to get tip").expect("no tip");
...
fn test_snapshot_list() {
    ...
    for height in [100, 200, 300] {
        let (db_dir, store) = create_test_db(height);
        manager.create_snapshot(&store, 1000).expect("failed to create snapshot");
...
fn test_snapshot_metadata_validation() {
    ...
    let err_msg = result.unwrap_err().to_string();
    assert!(
        err_msg.contains("tip hash") || err_msg.contains("Corrupted") || err_msg.contains("Invalid"),
...
fn test_snapshot_format_version() {
    ...
    let result = manager.restore_snapshot(&invalid_archive, &target);
    assert!(result.is_err(), "restore should reject unsupported format version");
    
    let err_msg = result.unwrap_err().to_string();
    assert!(
        err_msg.contains("format version") || err_msg.contains("999") || err_msg.contains("unsupported"),
