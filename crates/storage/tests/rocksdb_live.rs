...
fn test_batch_atomicity() {
    ...
    // Create a simple tip
    let tip = TipInfo::new(1, [42u8; 32], BigUint::from(1000u64), 0);
...
fn test_reopen_persistence() {
    ...
    {
        let store = Store::open_with_tuning(temp_dir.path(), tuning.clone()).unwrap();
        let tip = TipInfo::new(42, test_hash, BigUint::from(999999u64), 5);
        store.set_tip_meta(&tip).unwrap();
    }
...
fn test_write_reopen_read() {
    ...
    {
        let store = Store::open_with_tuning(temp_dir.path(), tuning.clone()).unwrap();
        let tip = TipInfo::new(100, test_hash, BigUint::from(12345u64), 0);
        store.set_tip_meta(&tip).unwrap();
    ...
    // Session 2: Reopen and read
    {
        ...
        let tip = store
            .tip()
            .unwrap()
            .expect("data should persist across restarts");
